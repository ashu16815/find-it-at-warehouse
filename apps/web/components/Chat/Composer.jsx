...
export default function Composer() {
  ...
  return (
    <div className="sticky bottom-4 z-40 mx-auto max-w-[1200px] px-6">
                      ...
                      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 mt-3">
                        {message.products.slice(0, 3).map((product, i) => (
                          <a
                            ...
                            className="block rounded-lg border border-border bg-bg hover:shadow-card transition-all"
                          >
                            {product.image && (
                            ...
                            <div className="p-3">
                              ...
                              <div className="flex items-center justify-between text-xs text-ink-muted">
                                ...
                                <span className="px-2 py-1 rounded-sm text-xs font-medium bg-brand-primary/10 text-brand-primary">TWG</span>
                              </div>
                            </div>
                          </a>
                        ))}
      ...
      {/* Input Form */}
      <form onSubmit={onSubmit} className="bg-white/95 backdrop-blur-sm rounded-2xl border border-border p-4 shadow-card">
        <div className="flex gap-3 items-center">
          <input
            ref={inputRef}
            key={messages.length} // Force re-render when messages change
            className="flex-1 outline-none text-base bg-transparent placeholder:text-ink-muted text-ink"
          ...
          <button 
            ...
            className={`px-6 py-3 rounded-full text-sm font-medium min-h-[44px] transition-all ${
              loading
                ...
                : 'bg-[#D32F2F] text-white hover:bg-[#B71C1C] focus:outline-none focus:ring-2 focus:ring-[#D32F2F] focus:ring-offset-2'
            }`}
          >
